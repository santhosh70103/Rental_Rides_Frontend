...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (AdminloginResponse.data.flag === true) {
        Navigate(`/Admindash/${AdminloginResponse.data.Id}`);
      } else {
        const CustomerLoginResponse = await axios.post(
          "https://localhost:7208/api/Customers/CustomerLogin",
          userData
        );
        if (CustomerLoginResponse.data.flag === true) {
          Navigate(`/Customerdash/${CustomerLoginResponse.data.Id}`);
        } else {
